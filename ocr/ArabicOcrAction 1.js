// ArabicOCR — single document export (multi-page PDF and image documents)
window.ARABIC_OCR_VERSION = "2026-08-04-single-export";
...
window.runArabicOcrAction = function () {
    ...
    // Laserfiche uses p=0 for the complete document; p=1 is the current page.
    // A single export preserves the original PDF/image payload and avoids one export
    // and two OCR HTTP calls for every page.
    ...
    .then(function (pdfBlob) {
        ...
        fieldsFormData.append("image", pdfBlob, "document.pdf");
        fieldsFormData.append("fields", Object.keys(fieldsById).join(","));

        var allTextFormData = new FormData();
        allTextFormData.append("image", pdfBlob, "document.pdf");

(function () {
        ...
        return parentWindow.fetch(url, {
            ...
            return response.text().then(function (text) {
                if (!response.ok) {
                    ...
                    var error = new Error("HTTP " + response.status + " calling " + url + detail);
                ...
                try { return JSON.parse(text); }
                    ...
                    throw new Error("Invalid JSON returned by " + url + ": " + text.slice(0, 160));
                }
            });
        });
    }
    ...
    function errorMessage(error, fallback) {
        if (error && (error.status === 401 || error.status === 403)) {
            return "لا يملك حسابك صلاحية تعديل بيانات المستند. امنح المستلمين صلاحية Write Metadata (WMe)، " +
                "ولا تضعها ضمن الحقوق المرفوضة، أو نفّذ التسجيل من Workflow بحساب workflow.";
        }
        if (error && error.status === 409) {
            return "المستند مستخدم أو مقفول حاليًا؛ ستتم إعادة المحاولة تلقائيًا.";
        }
        return (error && error.message) || fallback || "خطأ غير متوقع";
    }
    ...
    /* ينفّذ التعديل داخل قفل واحد ويضمن تحريره حتى عند فشل القراءة أو الحفظ. */
    function withDocumentLock(entryId, action) {
        var locked = false;
        return lock(entryId).then(function () {
            locked = true;
            return action();
        }).then(function (result) {
            return unlock(entryId).then(function () { return result; });
        }, function (error) {
            if (!locked) throw error;
            return unlock(entryId).then(function () { throw error; });
        });
    }
    ...
    function armTracking(context, recipients) {
        ...
        withDocumentLock(context.entryId, function () {
            return readFields(context.entryId).then(function (fields) {
    ...
    function autoAcknowledge(context) {
        if (saving) return;
        saving = true;
        var savedState = null;

        /* أعد القراءة بعد الحصول على القفل كي لا تمحو إقرار مستخدم آخر متزامن. */
        withDocumentLock(context.entryId, function () {
            return readFields(context.entryId).then(function (fields) {
                if (!fields.status) throw new Error("الحقل غير موجود في القالب: " + config.statusField);
                var state = parseLedger(fields.status.value || fields.status.Value || "");
                var position = indexOfUser(state.pending, context.userName);
                if (!state.armed || position === -1) return false;
                ...
                savedState = state;
                return save(context.entryId, fields.status.id, renderLedger(state)).then(function () { return true; });
            });
        }).then(function () {
            saving = false;
            ...
            if (savedState) {
                var total = savedState.done.length + savedState.pending.length;
                toast("تم تسجيل اطلاعك — " + savedState.done.length + " " + L.of + " " + total, false);
            }
        }).catch(function (error) {
            saving = false;
            var scope = states[context.scopeKey];
            ...
            toast(errorMessage(error, "تعذر تسجيل الاطلاع، ستتم إعادة المحاولة."), true);
        });
    }
